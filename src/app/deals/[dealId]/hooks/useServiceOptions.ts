...
async function fetchServiceOptions(): Promise<ServiceOption[]> {
  const response = await fetch(`/api/deals/service-options`);
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.message || "Failed to fetch service options");
  }
  return response.json();
}

export function useServiceOptions(initialOption?: string) {
  ...
  useEffect(() => {
    ...
    fetchServiceOptions()
      ...
      .catch((err: unknown) => {
        const message = err instanceof Error ? err.message : "Failed to load service options.";
        setError(message);
        toast.error(message);
        if (initialOption) {
          setOptions((prev) => {
